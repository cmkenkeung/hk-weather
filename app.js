...
async function fetchData() {
    ...
    try {
        // Fetch Current weather with no-store to prevent caching on refresh
        let currentRes = await fetch(API_CURRENT, { cache: "no-store" });
        if (!currentRes.ok) throw new Error("Current API returned " + currentRes.status);
        let currentData = await currentRes.json();
        
        // Fetch Forecast with no-store
        let forecastRes = await fetch(API_FORECAST, { cache: "no-store" });
...
function renderWarnings() {
    ...
    const currentData = appData.current || {};
    let warnMsg = currentData.warningMessage || "";
    
    // The API might return an array of warning messages
    if (Array.isArray(warnMsg)) {
        warnMsg = warnMsg.join(" ");
    }
    
    if (warnMsg && typeof warnMsg === 'string' && warnMsg.trim() !== "" && warnMsg.trim() !== "null") {
...
function renderHero() {
    ...
    if (tempValEl) {
        ...
        // Removed updating of nextElementSibling because the unit toggle is now there.
    }
